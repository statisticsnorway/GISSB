...
Ved å bruke disse funksjonene kan man bl.a. finne koordinater til adresser og beregne kjøretid- og avstand mellom disse ved å bruke vegnettet (NVDB Ruteplan nettverksdatasett). Dette kan være nyttig dersom man ønsker å analysere regionale forskjeller i befolkningens tilgang til offentlige tjenester.
...
Funksjonen `address_to_coords` bruker [Kartverkets adresse-API](https://ws.geonorge.no/adresser/v1/#/) for å finne koordinater til adresser. Denne API-en krever nettilgang og baserer seg på matrikkelen (Norges offisielle eiendomsregister som inneholder alle offisielle adresser med tilhørende koordinater). Ved å skrive inn postnummer og adresse returneres et [sf-objekt](https://r-spatial.github.io/sf/articles/sf1.html) med koordinatene til denne adressen.
...
graph_cppRouting_FT_MINUTES <- readRDS(file = "C:/Users/rdn/Documents/Kart/Vegnett/graph_cppRouting_FT_MINUTES.rds")
graph_cppRouting_LENGTH <- readRDS(file = "C:/Users/rdn/Documents/Kart/Vegnett/graph_cppRouting_LENGTH.rds")
...
![](unnamed-chunk-6-1.PNG){width=600px}
...
Objektet `graph_cppRouting_FT_MINUTES` benyttes for å beregne korteste kjøretid i minutter med funksjonen `shortest_path_cppRouting`. Objektet `graph_cppRouting_LENGTH` benyttes for å beregne korteste kjøreavstand i meter.
...
For å beregne korteste kjøretid i minutter med `shortest_path_cppRouting` settes `graph_cppRouting = graph_cppRouting_FT_MINUTES`, mens `graph_cppRouting = graph_cppRouting_LENGTH` beregner korteste kjøreavstand i meter. Objektet/objektene `graph_cppRouting_FT_MINUTES`/`graph_cppRouting_LENGTH` må være lastet inn (gjøres med funksjonen `vegnett_to_R`) før funksjonen `shortest_path_cppRouting` kan brukes.
