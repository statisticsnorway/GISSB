...
# source("C:/Users/rdn/Documents/Kart/GiSSB/adresse_api_koord.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/koords_to_google.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/vegnett_to_R.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/node_koord.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/beregne_avstand.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/path_leaflet.R", encoding = "UTF-8")
# source("C:/Users/rdn/Documents/Kart/GiSSB/beregne_avstand_cpp.R", encoding = "UTF-8")
...
For å få koordinatene med CRS 4326 må dette spesifiseres i funksjonen `adresse_api_koord`. Det er også mulig å angi en vektor med flere postnummere og adresser for å få koordinatene til flere adresser:
...
`nodes` og `edges` benyttes for å plassere koordinatene til sitt nærmeste punkt i vegnettet med funksjonen `node_koord`.
...
`graph_cppRouting_FT_MINUTES` benyttes for å beregne korteste kjøretid i minutter med funksjonen `beregne_avstand_cpp`. `graph_cppRouting_LENGTH` benyttes for å beregne korteste kjøreavstand i meter.
...
## Knytter koordinater til vegnettet (node_koord)
...
```{r, eval=TRUE}
from_node <- GISSB::node_koord(koords = fra, fra_til = "fra", membership = F)
from_node

to_node <- GISSB::node_koord(koords = til, fra_til = "til", membership = T)
to_node
```

## Beregner kjøreavstand med tidygraph (beregne_avstand)

Når man har en node-ID for både start- og stoppunkt kan man benytte funksjonen `beregne_avstand` for å beregne den korteste kjøreavstanden mellom disse. For å beregne korteste kjøreavstand i minutter setter man enhet = "FT_MINUTES" (default), mens enhet = "LENGTH" beregner korteste kjøreavstand i meter.

For å kjøre koden må objektet graph (som lages med funksjonen `vegnett_to_R`) være lastet inn.

OBS: `beregne_avstand` fungerer ikke med vektorer, dvs. liste over flere fra- eller tilnoder. Det er kun mulig å beregne avstander én til én. For å beregne avstand med flere fra- og/eller tilnoder, se `beregne_avstand_cpp`.

```{r, eval=TRUE}
avstand_min <- GISSB::beregne_avstand(from_node = from_node$from_node,
                           to_node = to_node$to_node,
                           enhet = "FT_MINUTES",
                           path = F)
...
avstand_meter <- GISSB::beregne_avstand(from_node = from_node$from_node,
                                 to_node = to_node$to_node,
                                 enhet = "LENGTH",
                                 path = F)
...
```{r, eval=TRUE}
path <- GISSB::beregne_avstand(from_node = from_node$from_node,
                           to_node = to_node$to_node,
                           enhet = "FT_MINUTES",
                           path = T)
...
til <- GISSB::adresse_api_koord(postnummer = postnummere,
                         adresse = adresser) %>%
  GISSB::koords_to_google()
to_node <- GISSB::node_koord(koords = til, fra_til = "til", membership = F)

fra <- GISSB::adresse_api_koord(postnummer = c("0177", "2211"),
                         adresse = c("Akersveien 26", "Otervegen 23")) %>%
  GISSB::koords_to_google()
from_node <- GISSB::node_koord(koords = fra, fra_til = "fra", membership = T)

avstand_cpp <- GISSB::beregne_avstand_cpp(unique(from_node$from_node),
                                       unique(to_node$to_node),
                                       enhet = "FT_MINUTES")
avstand_cpp

avstand_cpp_min <- GISSB::beregne_avstand_cpp(unique(from_node$from_node),
                                       unique(to_node$to_node),
                                       enhet = "FT_MINUTES",
                                       dist = "min")
avstand_cpp_min

avstand_cpp_max <- GISSB::beregne_avstand_cpp(unique(from_node$from_node),
                                       unique(to_node$to_node),
                                       enhet = "FT_MINUTES",
                                       dist = "max")
avstand_cpp_max
